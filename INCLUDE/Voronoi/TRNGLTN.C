...
void TriangulacionMonotona(Vertices *v)
{
	...
	if(esMonotono)
	{
		...
		while(!queue.vacio)
		{
			...
			{
				if(stack.elementos[stack.stackIndice].cadena == izquierda)
				{
					...
					else if(EnLaIzquierda(&stack.elementos[stack.stackIndice - 1], &stack.elementos[stack.stackIndice], &verticeActual))
					{
						...
						{
							DibujarSegmentoApartirDeVertices(&stack.elementos[stack.stackIndice - 1], &verticeActual, VCOLOR_LADO_POLI);
			...
			/* El 3er caso, el caso B, es cuando verticeActual esta en la cadena opuesta
				...
				verticeActual */
			else if(verticeActual.cadena != stack.elementos[stack.stackIndice].cadena && verticeActual.cadena != ambas)
			{
				top = stack.elementos[stack.stackIndice];
				/* conectamos todos los elementos del stack en bucle */
				while(stack.stackIndice > 0)
				{
					DibujarSegmentoApartirDeVertices(&stack.elementos[stack.stackIndice], &verticeActual, VCOLOR_LADO_POLI);
					StackVerticePop(&stack);
				}
